<template>
  <SidebarLayout :page-title="campaign.name" page-subtitle="Campaign Respondents">
    ...
    <div class="mb-6">
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-4 md:p-6">
        ...
        <p class="font-karla text-sm text-gray-600">
          {{ respondentList.length }} respondent(s) from {{ campaign.propertyCount }} properties
        </p>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="font-karla text-sm text-gray-600">
        {{ filteredRespondents.length }} respondent(s)
      </div>
      <select
    ...
    <div v-if="filteredRespondents.length === 0" class="bg-white rounded-lg p-8 border border-gray-200 text-center">
      <p class="font-karla text-gray-500">
        {{ statusFilter === 'all' ? 'No respondents yet for this campaign.' : 'No respondents with this status.' }}
      </p>
    </div>
...
function statusBadgeClass(status: string): string {
  switch (status) {
    ...
    case 'healthcheck_scheduled': return 'bg-purple-100 text-purple-700'
    case 'healthcheck_completed': return 'bg-green-100 text-green-700'
...
function formatStatus(status: string): string {
  switch (status) {
    ...
    case 'healthcheck_scheduled': return 'Scheduled'
    case 'healthcheck_completed': return 'Completed'
...
async function updateStatus(respondentId: number, newStatus: string) {
  try {
    ...
    if (response.ok) {
      const respondent = respondentList.value.find((r) => r.id === respondentId)
      if (respondent) respondent.status = newStatus
    } else {
      notification.value = { show: true, type: 'error', title: 'Error', message: 'Failed to update status' }
    }
  } catch {
    notification.value = { show: true, type: 'error', title: 'Error', message: 'An error occurred' }
  }
}

function openNotesDialog(respondent: Respondent) {
...
async function saveNotes() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      notification.value = { show: true, type: 'error', title: 'Error', message: 'Failed to save notes' }
    }
  } catch {
    notification.value = { show: true, type: 'error', title: 'Error', message: 'An error occurred' }
  }
