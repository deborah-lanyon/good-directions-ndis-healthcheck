<template>
  <SidebarLayout pageTitle="New Properties" pageSubtitle="Manage and track property visits">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
      ...
      <div class="flex flex-wrap items-center gap-2 md:gap-4 w-full md:w-auto">
        <Button
          ...
          class="flex-1 md:flex-none bg-green-600 hover:bg-green-700 text-sm md:text-base rounded-full"
        >
        ...
        <Button
          ...
          @click="planRoute"
          class="flex-1 md:flex-none bg-primary hover:bg-blue-700 text-sm md:text-base rounded-full"
        >
    ...
    <div class="flex flex-col gap-4">
      <div class="flex flex-wrap items-center gap-3 md:gap-6 w-full">
        ...
        >
          ...
          <option value="visited">Filter: Visited</option>
          <option value="not-visited">Filter: Not Visited</option>
        </select>
        <select
    ...
    <div>
      ...
      <section v-if="viewMode === 'list'" class="mt-8 flow-root">
        <div class="overflow-hidden border-2 border-black/5 sm:rounded-lg bg-white">
          <Table>
              ...
              >
                ...
                <TableCell class="whitespace-nowrap px-3 py-4 text-sm">
                  <div class="flex gap-2">
                    <Button
                      @click="generateWelcomePack(property.id, 'html')"
                      size="sm"
                      variant="outline"
                      class="inline-flex items-center gap-1.5 text-xs text-primary bg-blue-50 hover:bg-blue-100 border-blue-200"
                      title="Generate HTML Welcome Pack"
                    >
                      <Icon name="code" class="w-3.5 h-3.5" />
                      HTML
                    </Button>
                    <Button
                      @click="generateWelcomePack(property.id, 'pdf')"
                      size="sm"
                      variant="outline"
                      class="inline-flex items-center gap-1.5 text-xs text-red-600 bg-red-50 hover:bg-red-100 border-red-200"
                      title="Generate PDF Welcome Pack"
                    >
...
const { status: liveSyncStatus } = useSyncStatus({
  onSyncComplete: () => {
...
const visitedFilter = ref<VisitedFilter>('all')
...
const filteredProperties = computed(() => {
  ...
  if (visitedFilter.value !== 'all') {
    if (visitedFilter.value === 'visited') {
  ...
  if (streetGroupFilter.value !== 'all') {
    const selectedGroup = streetGroups.value.find((g) => g.id === streetGroupFilter.value)
    if (
      selectedGroup &&
  ...
  if (visitorFilter.value !== 'all') {
    const selectedVisitor = visitors.value.find((v) => v.id === visitorFilter.value)
    if (selectedVisitor) {
      filtered = filtered.filter(
...
onMounted(async () => {
  ...
  // Set visited filter from URL
  if (visitedParam === 'visited' || visitedParam === 'not-visited') {
    visitedFilter.value = visitedParam
  }
